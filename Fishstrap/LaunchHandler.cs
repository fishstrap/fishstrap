...
using Fishstrap.UI.Elements.Dialogs;
...
namespace Fishstrap
{
    public static class LaunchHandler
    {
        ...
        public static void ProcessLaunchArgs()
        {
            ...
            else if (App.LaunchSettings.DisableRobloxAutoStartFlag.Active)
            {
                App.Logger.WriteLine(LOG_IDENT, "Disabling Roblox startup");
                WindowsRegistry.DisableRobloxAutoStart(false);
                App.Terminate();
            }
            else if (App.LaunchSettings.EnableRobloxAutoStartFlag.Active)
            {
                App.Logger.WriteLine(LOG_IDENT, "Enabling Roblox startup");
                WindowsRegistry.EnableRobloxAutoStart(false);
                App.Terminate();
            }
        ...
        public static void LaunchInstaller()
        {
            ...
            {
#if QA_BUILD
                Frontend.ShowMessageBox("You are about to install a QA build of Fishstrap. The red window border indicates that this is a QA build.\n\nQA builds are handled completely separately of your standard installation, like a virtual environment.", MessageBoxImage.Information);
#endif
        ...
        public static void LaunchBackgroundUpdater()
        {
            ...
            {
                MutexName = "Fishstrap-BackgroundUpdater",
